...
# Script to start VM via SCVMM from Linux/Unix environment
...
try {
    ...
    if ($VM) {
        ...
        if ($VM.Status -eq "Running") {
            ...
            Start-SCVirtualMachine -VM $VM -ErrorAction Stop
            ...
            } while ($VM.Status -ne "Running" -and $elapsed -lt $timeout)
            
            if ($VM.Status -eq "Running") {
                Write-Host "VM successfully started"
            } else {
                Write-Warning "VM startup timeout reached. Current status: $($VM.Status)"
            }
        }
    } else {
...
echo "VM startup operation completed"
